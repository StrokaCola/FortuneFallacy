// bg-shader.js — WebGL animated cosmic nebula background
// Renders on a canvas layered behind the main game canvas.
// No external dependencies — raw WebGL + inline GLSL.

const VERT_SRC = `
attribute vec2 a_pos;
void main() { gl_Position = vec4(a_pos, 0.0, 1.0); }
`;

const FRAG_SRC = `
precision mediump float;

uniform vec2  u_res;
uniform float u_time;
uniform float u_mode;   // 0=calm  1=active-game  2=shop/hub  3=intense
uniform float u_flash;  // 0–1, momentary brightness hit for big scores

float hash(vec2 p) {
  return fract(sin(dot(p, vec2(127.1, 311.7))) * 43758.5453123);
}

float noise(vec2 p) {
  vec2 i = floor(p);
  vec2 f = fract(p);
  f = f * f * (3.0 - 2.0 * f);
  return mix(
    mix(hash(i),               hash(i + vec2(1.0, 0.0)), f.x),
    mix(hash(i + vec2(0.0,1.0)), hash(i + vec2(1.0, 1.0)), f.x),
    f.y
  );
}

float fbm(vec2 p) {
  float v = 0.0;
  float a = 0.5;
  vec2  shift = vec2(100.0);
  for (int i = 0; i < 5; i++) {
    v += a * noise(p);
    p  = p * 2.1 + shift;
    a *= 0.48;
  }
  return v;
}

void main() {
  vec2 uv = gl_FragCoord.xy / u_res;
  uv.y = 1.0 - uv.y;

  float t = u_time * 0.035;

  // Two passes of domain-warped fBm for nebula depth
  vec2 q  = uv * 2.8 - 1.4;
  float n1 = fbm(q + t * vec2(0.18, 0.12));
  float n2 = fbm(q + n1 * 0.85 + vec2(3.43, 2.14) + t * 0.06);
  float nebula = fbm(q + n2 * 0.65 + vec2(8.1, 4.3) + t * 0.03);

  // Per-mode color ramps — cold to warm
  // m=0 calm:   deep indigo
  // m=1 game:   violet-blue
  // m=2 shop:   cooler teal-blue
  // m=3 intense: blood-crimson (portent / boss — reserved for Ticket 4)
  float m = clamp(u_mode, 0.0, 3.0);

  vec3 cold, warm;
  // 0→1 blend
  cold = mix(vec3(0.015, 0.008, 0.045), vec3(0.030, 0.010, 0.090), clamp(m,      0.0, 1.0));
  warm = mix(vec3(0.045, 0.018, 0.120), vec3(0.120, 0.040, 0.220), clamp(m,      0.0, 1.0));
  // 1→2 blend
  cold = mix(cold, vec3(0.008, 0.025, 0.065), clamp(m - 1.0, 0.0, 1.0));
  warm = mix(warm, vec3(0.020, 0.060, 0.130), clamp(m - 1.0, 0.0, 1.0));
  // 2→3 blend
  cold = mix(cold, vec3(0.060, 0.004, 0.018), clamp(m - 2.0, 0.0, 1.0));
  warm = mix(warm, vec3(0.200, 0.018, 0.050), clamp(m - 2.0, 0.0, 1.0));

  vec3 col = mix(cold, warm, nebula * 0.95 + 0.03);

  // Subtle star field — bright pinpoints using high-power hash threshold
  vec2 sUV = uv * vec2(200.0, 112.0);
  float star = hash(floor(sUV));
  star = pow(max(0.0, star - 0.965) / 0.035, 3.0);
  // Twinkle
  float twinkle = 0.6 + 0.4 * sin(u_time * 1.4 + hash(floor(sUV)) * 6.28);
  col += star * twinkle * vec3(0.85, 0.90, 1.00) * 0.55;

  // Occasional brighter "hero" stars at lower resolution
  vec2 hUV = uv * vec2(55.0, 31.0);
  float hStar = hash(floor(hUV));
  hStar = pow(max(0.0, hStar - 0.96) / 0.04, 4.0);
  float hTwinkle = 0.5 + 0.5 * sin(u_time * 0.9 + hash(floor(hUV)) * 6.28);
  col += hStar * hTwinkle * vec3(0.90, 0.88, 1.00) * 0.80;

  // Vignette — draws the eye to centre
  float vig = 1.0 - smoothstep(0.25, 1.05, length((uv - 0.5) * vec2(1.78, 1.0)));
  col *= vig * 0.92 + 0.08;

  // Flash pulse for big-score moments
  col += u_flash * vec3(0.18, 0.12, 0.32) * smoothstep(0.3, 1.0, nebula);
  col += u_flash * 0.06 * vec3(1.0, 0.9, 1.0);

  gl_FragColor = vec4(col, 1.0);
}
`;

// ─── Module state ──────────────────────────────────────────────────────
let gl       = null;
let program  = null;
let uniforms = {};
let vbo      = null;
let _canvas  = null;
let _active  = false;
let _flash   = 0;

// Screen → shader mode mapping
const SCREEN_MODES = {
  title:     0,
  nameentry: 0,
  howto:     0,
  scores:    0,
  win:       1,
  game:      1,
  hub:       2,
  shop:      2,
  forge:     2,
  rune:      2,
};

// ─── Compile helper ────────────────────────────────────────────────────
function compileShader(type, src) {
  const s = gl.createShader(type);
  gl.shaderSource(s, src);
  gl.compileShader(s);
  if (!gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
    console.error('[bg-shader] compile error:', gl.getShaderInfoLog(s));
    gl.deleteShader(s);
    return null;
  }
  return s;
}

// ─── Public API ────────────────────────────────────────────────────────

/**
 * Initialise the WebGL context on the given canvas element.
 * Returns true if WebGL is available, false if we should fall back to CSS bg.
 */
export function initBg(canvas) {
  _canvas = canvas;
  try {
    gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
  } catch (_) { gl = null; }
  if (!gl) return false;

  const vs = compileShader(gl.VERTEX_SHADER,   VERT_SRC);
  const fs = compileShader(gl.FRAGMENT_SHADER, FRAG_SRC);
  if (!vs || !fs) return false;

  program = gl.createProgram();
  gl.attachShader(program, vs);
  gl.attachShader(program, fs);
  gl.linkProgram(program);
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    console.error('[bg-shader] link error:', gl.getProgramInfoLog(program));
    return false;
  }

  // Fullscreen clip-space quad (triangle-strip)
  vbo = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
  gl.bufferData(gl.ARRAY_BUFFER,
    new Float32Array([-1, -1,  1, -1,  -1, 1,  1, 1]),
    gl.STATIC_DRAW);

  uniforms = {
    res:   gl.getUniformLocation(program, 'u_res'),
    time:  gl.getUniformLocation(program, 'u_time'),
    mode:  gl.getUniformLocation(program, 'u_mode'),
    flash: gl.getUniformLocation(program, 'u_flash'),
  };

  _active = true;
  return true;
}

/** Returns true if the WebGL background is active. */
export function bgActive() { return _active; }

/**
 * Call from big-score / screen-flash moments to pulse the nebula.
 * amount is 0–1.
 */
export function flashBg(amount = 0.8) {
  _flash = Math.min(1.0, _flash + amount);
}

/**
 * Call once per animation frame from the main game loop.
 * @param {number} time   elapsed seconds (same `t` passed to draw functions)
 * @param {string} screen current game screen name
 */
export function tickBg(time, screen) {
  if (!_active) return;

  // Decay flash
  _flash = Math.max(0, _flash - 0.025);

  const mode = SCREEN_MODES[screen] ?? 0;
  const cw = _canvas.width;
  const ch = _canvas.height;

  gl.viewport(0, 0, cw, ch);
  gl.useProgram(program);

  gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
  const posLoc = gl.getAttribLocation(program, 'a_pos');
  gl.enableVertexAttribArray(posLoc);
  gl.vertexAttribPointer(posLoc, 2, gl.FLOAT, false, 0, 0);

  gl.uniform2f(uniforms.res,   cw, ch);
  gl.uniform1f(uniforms.time,  time);
  gl.uniform1f(uniforms.mode,  mode);
  gl.uniform1f(uniforms.flash, _flash);

  gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
}
