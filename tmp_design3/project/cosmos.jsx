...
function Die({ face = 1, size = 88, style = 'celestial', locked, scoring, runes = [], onClick, label, dim }) {
  ...
  // bag of three.js objects for cleanup + animation
  const sceneRef = useRef(null);
  // Gate everything on the boot-module promise so we don't trip over stale
  // globals from a previous reload.
  const [ready, setReady] = useState(false);

  useEffect(() => {
    let alive = true;
    const isReady = () => !!(window.__buildDieVisual && window.__makeEnvMap && window.THREE);
    const tick = () => { if (alive && isReady()) setReady(true); };
    if (window.__diceReady) {
      window.__diceReady.then(tick);
    }
    // belt-and-braces poll in case the module is in flight when we mount
    const id = setInterval(() => { if (isReady()) { clearInterval(id); tick(); } }, 50);
    setTimeout(() => clearInterval(id), 5000);
    return () => { alive = false; clearInterval(id); };
  }, []);

  // pull style data from physics-die for glow color (rune-bg ring, drop shadow)
  const S = ready && window.__DIE_STYLES?.[style];
  const haloHex = S ? '#' + new window.THREE.Color(S.halo).getHexString() : '#7be3ff';

  // Mount scene once style/size are set, rebuild on style change.
  useEffect(() => {
    if (!ready) return;
    ...
    scene.environment = window.__makeEnvMap(renderer);

    // Tighter framing — narrow FOV pushes the die forward so bevels read
    // even at 56–88px on screen. Camera is offset slightly above and to the
    // side so the chosen face still dominates the view but two adjacent
    // edges catch light, giving the die its 3D read.
    const camera = new THREE.PerspectiveCamera(22, 1, 0.1, 50);
    camera.position.set(1.0, 0.85, 5.2);
    camera.lookAt(0, 0, 0);

    // lights: violet ambient + warm rim + cool key. Brighter than the big
    // physics scene because each die has its own self-contained lighting.
    const amb = new THREE.AmbientLight(0x9577ff, 0.85);
    scene.add(amb);
    const key = new THREE.DirectionalLight(0xbba8ff, 1.4);
    key.position.set(-3, 4, 5);
    scene.add(key);
    const rim = new THREE.PointLight(0x7be3ff, 1.6, 18);
    rim.position.set(3, 1, -3);
    scene.add(rim);
    const fill = new THREE.PointLight(0xf5c451, 0.7, 16);
    fill.position.set(-3, -1, -2);
    scene.add(fill);

    // build the die mesh (translucent crystal cube w/ glowing pip orbs)
    const die = window.__buildDieVisual({ size: 1, style });
    scene.add(die);

    sceneRef.current = {
      THREE, renderer, scene, camera, die, amb, key, rim,
      raf: 0, born: performance.now(),
      faceRot: new THREE.Quaternion(),
    ...
    const tick = () => {
      ...
      if (!ctx) return;
      const t = (performance.now() - ctx.born) / 1000;

      // smooth rotation toward target (face snap)
      ctx.currentRot.slerp(ctx.targetRot, 0.16);

      // gentle idle wobble layered on top of the snap rotation
      const wob = ctx.tmpQuat.setFromEuler(
        ctx.tmpEuler.set(
          Math.sin(t * 0.9) * 0.06,
          Math.sin(t * 0.7 + 1.3) * 0.08,
          Math.sin(t * 0.5 + 2.1) * 0.04
        )
      );
      ctx.die.quaternion.copy(ctx.currentRot).multiply(wob);

      // scoring pulse — modulate rim light + scale subtly
      if (scoring) {
        const p = 0.5 + 0.5 * Math.sin(t * 4);
        ctx.rim.intensity = 1.0 + p * 1.6;
        const s = 1 + p * 0.04;
        ctx.die.scale.setScalar(s);
      } else {
        ctx.rim.intensity = 1.0;
        ctx.die.scale.setScalar(1);
      }
  ...
  // Compute target rotation when `face` changes — make the chosen face's
  // outward normal point toward +Z (world up-front). Because the camera is
  // offset (positioned above & to the side), the face appears slightly
  // foreshortened — we see two adjacent edges + the bevel, giving the die
  // its 3D read while keeping the requested face dominant.
  useEffect(() => {
    const ctx = sceneRef.current;
    if (!ctx) return;
    const THREE = ctx.THREE;
    const FN = window.__FACE_NORMALS;
    if (!FN) return;

    const targetN = new THREE.Vector3(0, 0, 1);
    const fromN = FN[Math.max(1, Math.min(6, face))].clone();
    const q = new THREE.Quaternion().setFromUnitVectors(fromN, targetN);
    ctx.targetRot.copy(q);
    if (ctx.currentRot.x === 0 && ctx.currentRot.y === 0 && ctx.currentRot.z === 0 && ctx.currentRot.w === 1) {
      ctx.currentRot.copy(q);
    }
  }, [face, ready]);
  ...
  return (
    <div
      ...
      style={{
        ...
        opacity: dim ? 0.45 : 1,
        transform: locked ? 'translateY(-12px)' : 'none',
        transition: 'transform 220ms cubic-bezier(.4,1.4,.5,1)',
        filter: scoring ? `drop-shadow(0 0 14px ${haloHex})` : `drop-shadow(0 6px 12px rgba(0,0,0,0.5))`,
        userSelect: 'none',
      }}
    >
      <canvas
      ...
      <div style={{
        ...
        width: size * 0.7, height: 10, transform: 'translateX(-50%)',
